...
section_t* buildSectionStructure(uint8_t numOfCoefficients, char **testCoefficients);

int main(int argc, char *argv[]){
	section_t *aSection;
	
	uint8_t numOfCoefficients = 2;
	char testCoefficients[2][10] = {"1000_1001", "0010_1100"};
	
	aSection = (section_t*)buildSectionStructure(numOfCoefficients, &testCoefficients);
	
	
	/* Test Filter Structure */
	printf("Number of Multipliers: %d\n" , aSection->numOfMultipliers);
	printf("Number of Delays: %d\n" , aSection->numOfDelays);
	printf("Number of Adders: %d\n", aSection->numOfAdders);
	printf("Number of Edges: %d\n", aSection->numOfEdges);


	printf("\nMultipliers:\n");
	for(int i = 0; i < aSection->numOfMultipliers; i++){	
		printf("Muliplier(%d) ID: %s\n", i, aSection->multipliers[i].ID);
	}
	
	printf("\nAdders:\n");
	
	for(int i = 0; i < aSection->numOfAdders; i++){
		printf("Adder(%d) ID: %s\n", i, aSection->adders[i].ID);
	}
	
	printf("\nDelays:\n");

	for(int i = 0; i < aSection->numOfDelays; i++){
		printf("Delay(%d) ID: %s\n", i, aSection->delays[i].ID);
	}
	
	printf("\nEdges:\n");

	for(int i = 0; i < aSection->numOfEdges; i++){
		printf("Edge(%d) ID: %s\n", i, aSection->edges[i]->ID);
	}
...
section_t* buildSectionStructure(uint8_t numOfCoefficients, char **testCoefficients){
	...
	//Convient index that keeps track of what edges are already allocated in datastructure
	...
	adders = (adder_t*)malloc(sizeof(adder_t) * (numOfMultipliers - 1));
	...
	for(int i = 0; i < numOfMultipliers; i++){
		char *multiID = (char*)malloc(sizeof(char) * (12 + 4));
	
		// (12 + 4) = Multi_ddd + 3 for _in + 1 for null terminatior	
		char *inputPinID =  (char*)malloc(sizeof(char) * (12 + 4 + 4));
		
		// (12 + 4) = Multi_ddd + 4 for _out + 1 for null terminatior
		char *outputPinID = (char*)malloc(sizeof(char) * (12 + 4 + 5));
	...
	for(int i = 0; i < numOfDelays; i++){
		...
		char *delayID = (char*)malloc(sizeof(char) * (12 + 4));
	...
	for(int i = 0; i < numOfAdders; i++){
		...
		char *adderID = (char*)malloc(sizeof(char) * (12 + 4));
	
		char *inputPin0ID = (char*)malloc(sizeof(char) * (12 + 4 + 4));
		char *inputPin1ID = (char*)malloc(sizeof(char) * (12 + 4 + 4));
	...
	for(int i = 0; i < numOfDelays; i++){
		...
		curID = (char*)malloc(sizeof(char) * (37));
	...
	char *sectionInputID = (char*)malloc(sizeof(char) * (37));
	char *sectionOutputID = (char*)malloc(sizeof(char) * (37)); 	
