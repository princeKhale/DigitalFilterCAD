...
int main(int argc, char *argv[]){
	...
	bSection = (section_t*)buildSectionStructure(numOfBCoefficients, testBCoefficients);
	...
	for(int i = 0; i < aSection->numOfMultipliers; i++){	
		printf("Multiplier(%d) ID: %s\n", i, aSection->multipliers[i].ID);
		printf("	Coefficient:%s\n", aSection->multipliers[i].coefficient);
	}
	...
	writeVerilog(aSection, bSection);
